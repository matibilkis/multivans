...
circuit_db_craft = translator.initialize(mode="u2")
circuit_craft, circuit_db_craft = translator.give_circuit(circuit_db_craft)
translator.noise_model_params = {"pflip":7.61e-2,"1qb":11.68e-4,"2qb":3.17e-2,"t1":56e-9,"t2":56.01e-9,"tg":[]}

t1 = translator.noise_model_params["t1"]
t2 = translator.noise_model_params["t2"]




{"1":[cirq.DepolarizingChannel(p=translator.noise_model_params["1qb"]), cirq.PhaseDampingChannel()}

noisy_circuit = []
for k in circuit_craft.all_qubits():
    noisy_circuit.append(cirq.BitFlipChannel(p=translator.noise_model_params["pflip"]).on(k))

cirq.ResetChannel().with_probability(.01).on(q)

for k in list(circuit_craft.all_operations()):
    which = len(k.qubits)
    for q in k.qubits:
        noisy_circuit.append(cirq.DepolarizingChannel(p=0.01).on(q))
        noisy_circuit.append(cirq.PhaseFlipChannel(p=0.01).on(q))
        noisy_circuit.append(cirq.PhaseDampingChannel(.01).on(q))
        noisy_circuit.append(cirq.ResetChannel().with_probability(.01).on(q))

    noisy_circuit.append(k)

circuit_craft = cirq.Circuit(noisy_circuit)
tfq.convert_to_tensor([circuit_craft])
cirq.ResetChannel(0.01)

from tensorflow_quantum.core.serialize import serializable_gate_set

tfq.util.get_supported_channels()

help(cirq.AmplitudeDampingChannel)


help(cirq.GeneralizedAmplitudeDampingChannel)



help(cirq.PhaseDampingChannel)
