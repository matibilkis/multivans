...
indij=0
stru = {}
for j,k in list(evals.items()):
    ii = np.argmin(cost_vans[j])
    stru[j] = database.describe_circuit(k[ii], k[ii].evolution[k[ii].get_best_iteration()][0])
...
np.save("data_8qb/opts_vans1",opp)
